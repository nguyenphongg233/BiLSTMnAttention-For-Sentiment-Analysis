...
\State \textbf{Input: $x^0 \in C$; $\lambda \in (0, 2/L)$; $k=0$ }
\State \textbf{Output: Stationary point $x^*$}

\While{true}
    \State $x^{k+1} \gets P_C\big(x^k - \lambda \nabla f(x^k)\big)$
    \If{$x^{k+1} = x^k$}
        \State \textbf{break}
    \EndIf
...
\begin{frame}{\textbf{\large 2.2.3 Chứng minh hệ quả 1}}    \vspace*{-1.5cm}    \textbf{1. Thiết lập bài toán:}
    Gọi $x^*$ là nghiệm tối ưu. Đặt $\Delta_k := f(x^k) - f(x^*) \ge 0$.

    \textbf{2. Các đánh giá cơ bản:}
    Từ (\ref{eq:4}) kết hợp $x^k-x^{k+1} = \lambda \nabla \ f(x^k)$, ta có:
