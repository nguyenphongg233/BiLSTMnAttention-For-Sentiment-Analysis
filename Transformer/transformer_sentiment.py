...
class TransformerBlock(Layer):
    """
    Khối Transformer cơ bản (Transformer Block)
    Gồm: Multi-Head Attention -> Add & Norm -> Feed Forward -> Add & Norm
    """
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1, **kwargs):
        super(TransformerBlock, self).__init__(**kwargs)
        # Khởi tạo các sub-layers
...
class TokenAndPositionEmbedding(Layer):
    """
    Kết hợp Vector từ (Word Embedding) và Vector vị trí (Positional Embedding).
    Vì Transformer đọc song song, nên phải thêm vị trí để nó biết thứ tự từ.
    """
...
def build_transformer_model(vocab_size=MAX_WORDS):
    """
    Xây dựng mô hình Transformer từ đầu bằng Keras
    
    Kiến trúc:
        Input → TokenAndPositionEmbedding → TransformerBlock → Pooling → Dense → Softmax
    """
    print("\n" + "=" * 60)
    print("BƯỚC 2: XÂY DỰNG MÔ HÌNH TRANSFORMER")
    print("=" * 60)
    ...
    # ----- Transformer Block -----
    # Sử dụng 4 Attention Heads, và lớp ẩn Feed Forward = 128
    ...
    # ----- Global Pooling -----
    # Gom ma trận (batch, sequence, features) thành vector duy nhất (batch, features)
...
print("╔" + "═" * 58 + "╗")
print("║   SENTIMENT ANALYSIS - Transformer Architecture          ║")
