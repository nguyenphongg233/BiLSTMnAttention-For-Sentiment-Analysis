...
3. **Transformer Block**: Khối xử lý cốt lõi của Transformer:
   - **Multi-Head Attention**: Gồm 4 "đầu" (heads) hoạt động song song. Giúp mô hình tập trung vào các phần khác nhau của câu văn cùng lúc để hiểu ngữ cảnh phức tạp và mối quan hệ giữa các từ dù chúng ở xa nhau.
...
4. **Global Average Pooling 1D**: Gom cụm toàn bộ đặc trưng chuỗi đã qua xử lý thành một vector duy nhất, làm phẳng cấu trúc dữ liệu để chuyển sang các lớp Dense.
